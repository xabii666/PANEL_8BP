...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!secret || secret !== envPass) return NextResponse.json({ error: "Forbidden" }, { status: 403 })
    if (!username || !new_password) return NextResponse.json({ error: "Missing username or new_password" }, { status: 400 })
